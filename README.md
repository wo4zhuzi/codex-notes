...
3. 新项目先参考 [AI 项目落地清单](./ai-project-checklist.md)，确认导航、验证、权限边界和变更记录。
4. 阅读 [Git 融入 AI 工作流](./git-workflow.md)，明确分支、worktree、commit 和 push 的授权边界。
5. 新项目或高风险任务先参考 [Spec 模式实践](./spec-workflow.md)，确认边界、方案和进度。
6. Spec 确认后，核心业务逻辑参考 [TDD 实践手册](./tdd-workflow.md)，先用测试锁定行为再实现。
7. 再阅读 [Codex CLI 使用笔记](./codex-cli.md)，完成 Codex CLI 安装与登录。
8. 继续阅读 [Codex Skills 使用笔记](./codex-skills.md)，了解 skills 的触发方式和排查流程。
9. 如需理解知识库问答和检索增强生成，阅读 [RAG 使用笔记](./rag.md)，并运行 `demos/rag-notes/`。
10. 如需接入外部工具或文档服务，阅读 [Codex MCP 使用笔记](./mcp.md)。
11. 如需让 AI 使用 CodeGraph 等外部工具，阅读 [外部工具使用笔记](./external-tools)。
12. 阅读 [CC-Switch 使用笔记](./cc-swtich.md)，安装 CC-Switch 并了解配置切换方式。
13. 根据实际任务选择 `cc-switch-configs/` 中的配置模板。
14. 修改配置前建议保留原始配置备份，方便回滚。
