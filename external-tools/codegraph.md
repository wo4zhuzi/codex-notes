...
进入目标项目根目录并启动 Codex 后，可以直接要求 Codex 先通过 CodeGraph MCP 查询结构，再决定需要读取哪些源码。这样可以减少一开始大范围读文件造成的 token 消耗。

### 项目理解

```text
先用 codegraph 分析当前项目的核心模块、入口点、热点函数和主要依赖关系。不要先大范围读取文件。基于 CodeGraph 结果给我一份项目结构概览，并指出后续理解这个项目最应该看的 3 到 5 个文件或函数。
```

### 改代码前

```text
我要修改 <目标函数或模块>。先用 codegraph 查询它的定义位置、调用方、被调用方和影响面。不要直接大范围读文件。基于查询结果给出最小修改方案，说明需要改哪些文件、验证哪些场景，然后再实现。
```

### bug 排查

```text
这个 bug 可能和 <函数、模块或现象> 有关。先用 codegraph 查询相关入口函数、调用链、被调用方和影响面，再结合少量必要源码定位根因。不要一开始全仓库搜索或大范围读文件。
```

### 重构前

```text
我准备重构 <函数、类型或模块>。先用 codegraph 查询所有调用方、跨模块依赖、核心数据流和可能受影响的入口点。请先给出风险评估和分步重构计划，不要直接改代码。
```

### 代码审查

```text
先用 codegraph 分析 staged diff 的结构影响，包括变更函数、调用方、被调用方和跨模块影响面。再做 code review，重点看行为回归、遗漏测试、边界条件和是否需要同步更新文档。
```

### 省 token 用法

```text
这次任务请优先使用 codegraph 缩小范围：先查符号、调用链和影响面，只读取与修改直接相关的源码文件。不要为了理解背景而一次性读取大量无关文件。
```
...
```text
先用 codegraph 查询目标函数的定义、调用方、被调用方和影响面。不要直接大范围读文件。基于查询结果给出修改计划，然后按最小变更实现。
```
